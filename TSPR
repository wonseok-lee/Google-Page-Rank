...
def tspr_trans():
    ...
    for line in rough_topicID:
        temp = line.split(" ")
        topic_row.append(int(temp[1]) - 1)  # topic id = 12
        topic_col.append(int(temp[0]) - 1)  # doc id = 81433
...
def ptspr_off():
    ...
    for i in range(0, topic_num):
        ...
        for j in range(1, iter_num + 1):
            ...
            # temp_new = alpha*transmat*temp_old + alpha*trans_0 + beta*p_t + gamma*p_0
            ...
            if pow(temp_new - initvec, 2).sum(axis=0) ** 0.5 < error:
                break
            initvec = temp_new
        topic_pagerank.append(initvec)  # 12x81433 matrix
    end_ptspr = time.time()

    ptsprT = end_ptspr - start_ptspr

    print("PTSPR takes %s seconds" % ptsprT)

    return topic_pagerank


def ptspr_on():
    usertopicpath = "data/user-topic-distro.txt"
    ...
    topic_pagerank = ptspr_off()


    ptspr = userIDp_prob.dot(topic_pagerank)
    record_ptspr(ptspr[1])
...
def record_ptspr():
    with open("PTSPR-U2Q1.txt", "w") as f:
        document_ID = 0
        ptspr = ptspr_on()
        for i in ptspr:
            document_ID += 1
